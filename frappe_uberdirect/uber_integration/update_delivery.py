"""Background job updating the status of a delivery."""
...
def update_delivery(delivery_id: str, payload: dict = None) -> dict:
    ...
    if response.status_code != 200:
        msg = f"Failed to update delivery. Status code: {response.status_code}. Response: {response.text}"
        frappe.throw(msg=msg, exc=frappe.ValidationError)
